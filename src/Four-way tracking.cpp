...
// 定义传感器读数变量（使用新名称）
volatile int sensorValue1 = 0; //外线读数
volatile int sensorValue2 = 0; //内线读数
volatile int sensorValue3 = 0; //内线读数
volatile int sensorValue4 = 0; //外线读数
...
void tracing()
{
  ...
  if(sensorValue1 > 500 && sensorValue2 <= 500 && sensorValue3 <= 500 && sensorValue4 > 500){
    // Serial.println("直行");
    motorRun.Event_Judgment(ZHIXING);
  }
  // 左转（只有S2在黑线上）
  else if(sensorValue2 <= 500 && sensorValue3 > 500){
    // Serial.println("左转");
    motorRun.Event_Judgment(ZUOZHUAN);
  }
  // 右转（只有S3在黑线上）
  else if(sensorValue2 > 500 && sensorValue3 <= 500){
    // Serial.println("右转");
    motorRun.Event_Judgment(YOUZHUAN);
  }
  // 十字路口（全部在黑线上）默认直行
  ...
  // 其他情况
    ...
    motorRun.Event_Judgment(TINGZHI);
  }
}